...
void update_pony(DbConnection db, const Pony& pony) {
	...
	const char* sql =
		"UPDATE ponies SET name = ?, health = ?, max_health = ?, min_damage = ?, max_damage = "
		"?, attack_speed = ?, armor = ?, health_regeneration = ? WHERE id = ?;";
...
Pony create_pony(DbConnection db, const std::string& name) {
	sqlite3_stmt* stmt = nullptr;
	const char* sql =
		"INSERT INTO ponies (name, health, max_health, min_damage, max_damage, attack_speed, "
		"armor, health_regeneration) VALUES (?, 10, 0, 0, 0, 0, 0, 0);";
	int res = sqlite3_prepare_v2(*db, sql, -1, &stmt, nullptr);
	if (res != SQLITE_OK) throw std::runtime_error(sqlite3_errmsg(*db));
